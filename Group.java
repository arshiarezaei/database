...
public class Group {
    ...
    public void leave_group(String ID, String user){
        ...
        try {
            PreparedStatement preparedStatement = dbConnection.connection.prepareStatement("DELETE FROM group_MEMBERS WHERE group_id = (?) AND member_id = (?)");
    ...
    public void view_group(String groupID){
        try {
            PreparedStatement preparedStatement = dbConnection.connection.prepareStatement("SELECT * FROM group_messages WHERE group_ID = (?) ORDER BY DATE LIMIT 20");
    ...
    public void view_group_profile(String groupID){
        try {
            ...
            preparedStatement = dbConnection.connection.prepareStatement("SELECT * FROM groupS WHERE group_ID = (?)");
            preparedStatement.setString(1, groupID);
            ...
            preparedStatement = dbConnection.connection.prepareStatement("SELECT COUNT(*) FROM group_MEMBERS WHERE group_ID = (?)");
            preparedStatement.setString(1, groupID);
            rs = preparedStatement.executeQuery();
            String count = rs.getString(1);
            if(creator == null) {
                System.out.printf("Name:\"%s\" MembersCount:%s Link:\"%s\" Creator:\"%s\" Members:", name, count, link, adminPhone);
                preparedStatement = dbConnection.connection.prepareStatement("SELECT * FROM group_MEMBERS WHERE group_ID = (?)");
                preparedStatement.setString(1, groupID);
                rs = preparedStatement.executeQuery();
